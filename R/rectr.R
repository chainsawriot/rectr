...
#' @param corpus a multilingual corpus generated by create_corpus()
#' @param multiplication_factor integer, select k * mulitiplication_factor columns from the SVD-ed dfm.
#' @param dimension integer, the first singular value to be extracted.
#' @return an rectr_dfm object
#' @export
filter_dfm <- function(input_dfm, k, corpus, multiplication_factor = 2, dimension = 100) {
    svd_dfm <- RSpectra::svds(input_dfm$dfm, k = dimension, nu = dimension, nv = dimension)$u
    lang_vector <- quanteda::docvars(corpus, "lang")
    i <- .check_lang_indep(svd_dfm, lang_vector)
