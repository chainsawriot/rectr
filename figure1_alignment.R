...
scaled$points %>% as_tibble %>% mutate(word = c(en_examples$X1, de_examples$X1), lang = ifelse(c(en_examples$lang, de_examples$lang) == "en", "en", "de")) %>% ggplot(aes(x = V1, y = V2, col = lang)) + geom_point() + geom_text_repel(aes(label = word), size = 5) + xlab("Scaled vector space 1") + ylab("Scaled vector space 2") + scale_color_brewer(palette = "Dark2")
